...
if ( ! function_exists( 'business_ecommerce_fonts_url' ) ) :
	...
	function business_ecommerce_fonts_url() {
		...
		if ( 'off' !== $typography ) {
			...
			$font_families[] = get_theme_mod('header_font','Roboto').':300,400,500';
			$font_families[] = get_theme_mod('body_font','PT Sans').':300,400,500';
...
function business_ecommerce_admin_notice_info() {
    ...
    $message = __( 'Use Appearance > Customize > Theme Options to manage Sliders, Hero Content, Product Categories, Top Banner and Header | Footer options', 'business-ecommerce' );
 	$dismiss = __( 'Always Hide this Notice', 'business-ecommerce');
    printf( '<div class="%1$s"> <p><strong><span>%2$s</span></strong> &nbsp;&nbsp; <em><a href="?never_show_info" target="_self"  class="dismiss-notice">%3$s</a></em> </p></div>', esc_attr( $class ), esc_html( $message ), esc_html( $dismiss ) ); 
}
